	...
	if (document.querySelector('.child-add')) {
		...
		button.addEventListener('click', () => {
			...
			button.parentElement.insertAdjacentHTML(
				'beforeend',
				`      
         <div class="_child" style="order: ${
				button.parentElement.children.length - 1
			}; margin-top: 7px;">
...
$(function () {
	...
	$('.tours__slider').slick({
		...
		speed: 600,
		prevArrow: `
      <button class="slick-prev slick-arrow tours__prev" aria-label="Prev" type="button">
         <img src="img/left-arrow.svg" />
      </button>`,
		nextArrow: `
      <button class="slick-next slick-arrow tours__next" aria-label="Next" type="button">
	...
	$('.slider-images__items-thumbs').on(
		'beforeChange',
		function (event, slick, currentSlide, nextSlide) {
			...
			if (direction == 'right') {
				$(
					'.slick-cloned[data-slick-index="' + (nextSlide + slideCountZeroBased + 1) + '"]',
					$('.slider-images__items-thumbs')
	...
	$('.slider-images__items-thumbs').on('afterChange', function () {
		$('.slick-current-clone-animate', $('.slider-images__items-thumbs')).removeClass(
